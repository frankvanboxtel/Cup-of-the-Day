...
- `npm run generate:aliases` reads the generated JSON files, proposes player alias additions in `results/player-alias-proposals.json`, and writes generated alias groups to `data/player-aliases.generated.json`
- `npm run generate:html` reads the JSON files in `results/`, combines `data/player-aliases.json` with `data/player-aliases.generated.json`, and builds a static HTML site in `html/`
...
- `data/player-aliases.json` is for manually curated aliases
- `data/player-aliases.generated.json` is generated by `npm run generate:aliases`
